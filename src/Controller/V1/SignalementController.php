...
class SignalementController extends AbstractController
{
    ...
    #[Route('', name: 'list_signalements', methods: ['GET'])]
    #[IsGranted('ROLE_USER')]
    ...
    #[Route('/{id}', name: 'get_signalement', methods: ['GET'])]
    #[IsGranted('ROLE_USER')]
    public function getSignalement(Signalement $signalement): JsonResponse
    {
        ...
        if ($user->getUserRole() === UserRole::CITIZEN && $signalement->getUser() !== $user) {
            throw $this->createAccessDeniedException('You can only access your own signalements');
        }
    ...
    #[Route('', name: 'create_signalement', methods: ['POST'])]
    #[IsGranted('ROLE_USER')]
    public function createSignalement(Request $request): JsonResponse
    {
        ...
        if ($user->getUserRole() !== UserRole::CITIZEN) {
            return $this->json(
                ['success' => false, 'error' => 'Only citizens can create signalements'],
    ...
    #[Route('/{id}', name: 'update_signalement', methods: ['PUT'])]
    #[IsGranted('ROLE_USER')]
    public function updateSignalement(Signalement $signalement, Request $request): JsonResponse
    {
        ...
        // Check permissions
        if ($user->getUserRole() === UserRole::CITIZEN && $signalement->getUser() !== $user) {
            throw $this->createAccessDeniedException('You can only update your own signalements');
        }
        ...
        if ($user->getUserRole() === UserRole::CITIZEN && in_array($signalement->getStatus(), [SignalementStatus::RESOLVED, SignalementStatus::REJECTED])) {
            return $this->json(
                ['success' => false, 'error' => 'Cannot update resolved or rejected signalements'],
                Response::HTTP_BAD_REQUEST
            );
        }
        ...
        if (isset($data['title'])) {
            if (strlen($data['title']) < 5 || strlen($data['title']) > 255) {
                return $this->json(
                    ['success' => false, 'error' => 'Title must be between 5 and 255 characters'],
                    Response::HTTP_BAD_REQUEST
                );
            }
    ...
    #[Route('/{id}', name: 'delete_signalement', methods: ['DELETE'])]
    #[IsGranted('ROLE_USER')]
    public function deleteSignalement(Signalement $signalement): JsonResponse
    {
        ...
        if ($user->getUserRole() === UserRole::CITIZEN) {
            if ($signalement->getUser() !== $user) {
                throw $this->createAccessDeniedException('You can only delete your own signalements');
            }
            if ($signalement->getStatus() !== SignalementStatus::NEW) {
                return $this->json(
                    ['success' => false, 'error' => 'You can only delete NEW signalements'],
                    Response::HTTP_BAD_REQUEST
                );
            }
        }
        ...
        return $this->json(['success' => true, 'message' => 'Signalement deleted successfully']);
    }

    #[Route('/{id}/status', name: 'update_signalement_status', methods: ['PATCH'])]
